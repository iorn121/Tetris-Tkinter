...
def game():
    print("game starting...")
    global win, cv, score_label, speed_label
    win = Tk()  # ウィンドウの作成
    win.title("Tetris")
...
def play_game_over():
    os.system("afplay " + resource_path("game_over.wav"))


def play_delete():
    os.system("afplay " + resource_path("delete.wav"))


def thread_play_delete():
    t = multiprocessing.Process(target=play_delete)
    t.start()
